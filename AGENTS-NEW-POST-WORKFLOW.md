...
如果文章需要插入示意圖或封面圖（通常放在主標題下方、摘要內容上方）：

1. **AI 圖片生成**：
   使用 `generate_image` 工具生成符合語意的橫幅圖片（建議 prompt 包含 `16:9 ratio`、`minimalist clean vector style` 以利網頁呈現，若需包含中文可指定中文文字，但會有一定亂碼機率）。
2. **圖片壓縮最佳化（確保容量不超過 150KB）**：
   產生的 PNG 圖片檔案通常偏大，必須使用本機 `.NET` 方法將其轉換為 JPEG，並設定 **`Quality = 80`**：
   ```powershell
   Add-Type -AssemblyName System.Drawing
   $src = [System.Drawing.Image]::FromFile("C:\path\to\generated.png")
   ...
   $src.Save("source/images/your-image-slug.jpg", $encoder, $encoderParams)
   $src.Dispose()
   ```
3. **在 Markdown 中引用**：
   ```markdown
   ![圖片說明](../images/your-image-slug.jpg)
   ```
...
* **在 Linux/Mac 上**，可用 `sed`：
  ...
  ```
* **在 Windows (PowerShell) 上**，必須使用 .NET 類別 safe 地以 **無 BOM UTF-8** 寫入：
  ```powershell
